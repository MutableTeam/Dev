  ...
  const createLiquidityPool = async () => {
    ...
    try {
      ...
      if (balance !== null && solAmount > balance) {
        toast({
          title: "Error",
          description: "Insufficient SOL balance for pool creation",
          variant: "destructive",
        })
        return
      }
      ...
      if (mutbAmount > mutbBalance) {
        toast({
          title: "Error",
          description: "Insufficient MUTB balance for pool creation",
          variant: "destructive",
        })
        return
      }
    ...
    <Card className="bg-[#fbf3de] border-2 border-black shadow-[4px_4px_0px_0px_rgba(0,0,0,1)]">
      ...
      <CardContent>
        {checkingTradability ? (
          <div className="flex items-center justify-center p-4">
            ...
            <span>Checking if {MUTB_TOKEN.symbol} token is tradable on Jupiter...</span>
          </div>
        ) : !isTokenTradable ? (
      ...
      <CardFooter>
        {activeTab === "pool" && !isTokenTradable && (
          <button
            className="w-full bg-[#FFD54F] hover:bg-[#FFCA28] text-black border-2 border-black shadow-[2px_2px_0px_0px_rgba(0,0,0,1)] hover:shadow-[1px_1px_0px_0px_rgba(0,0,0,1)] hover:translate-x-[1px] hover:translate-y-[1px] transition-all font-mono p-2 rounded-md"
